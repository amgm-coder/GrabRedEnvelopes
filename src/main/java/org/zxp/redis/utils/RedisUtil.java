...
public class RedisUtil {
    ...
    /**
     * 只有在 key 不存在时设置 key 的值并设置超时时间 2019
     ...
     * 按照随机范围内超时的分布式锁  2019
     * @param lockKey
     * @param maxextime 最大随机数
     * @param minextime 最小随机数
     * @return
     */
    public boolean getLockRam(String lockKey,int maxextime,int minextime){
        Random random = new Random();
        int r = random.nextInt(maxextime) % (maxextime - minextime + 1) + minextime;
        return getLock(lockKey,String.valueOf(new Date().getTime()),r,SECONDS);
    }
     ...
     * @param timeout
     */
    public boolean getLockLua(String lockKey, String value, String timeout){
        DefaultRedisScript<Boolean> redisScript;
        redisScript = new DefaultRedisScript();
        redisScript.setScriptSource(new ResourceScriptSource(new ClassPathResource("setnxex.lua")));
        redisScript.setResultType(Boolean.class);
        List<String> keyList = new ArrayList<>();
        keyList.add(lockKey);
        keyList.add(value);
        return redisTemplate.execute(redisScript, keyList ,timeout);
    }
     ...
     * @return
     */
    public Object excuteLua(DefaultRedisScript redisScript,String... inner){
        List<String> list = Arrays.asList(inner);
     ...
     * @return
     */
    public String[] getTwo(String lockKey1, String lockKey2){
        DefaultRedisScript<String> redisScript;
        redisScript = new DefaultRedisScript();
        redisScript.setScriptSource(new ResourceScriptSource(new ClassPathResource("gettwo.lua")));
        redisScript.setResultType(String.class);
        List<String> keyList = new ArrayList<>();
        keyList.add(lockKey1);
        keyList.add(lockKey2);
        String result = redisTemplate.execute(redisScript, keyList );
        String[] results = new String[2];
        if(result != null && result.contains("SPLIT")){
            results[0] = result.split("SPLIT")[0];
            results[1] = result.split("SPLIT")[1];
            return results;
        }else {
            return null;
        }
     ...
     * @return
     */
    public Boolean bloomFilterAddLua(String bloomFilterName,String value){
    ...
    /**
     * Lua方式布隆过滤器exists 2019
     * @param bloomFilterName
     * @param value
     * @return
     */
    public Boolean bloomFilterExistsLua(String bloomFilterName,String value){
